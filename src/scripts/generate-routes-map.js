...
function generateRoutesMap(routesPath = './src/app/routes', outputDir = './src/core/tmp') {
	// 라우트 디렉토리 패스 설정 (Windows 경로 처리)
	const ROUTES_DIR = path.resolve(process.cwd(), routesPath);
	const TMP_DIR = path.resolve(process.cwd(), outputDir);
...
`;
	...
	if (!fs.existsSync(outputFileDir)) {
		...
		console.log(`📁 Created tmp directory: ${outputFileDir}`);
	}
